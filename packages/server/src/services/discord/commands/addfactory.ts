...
import { getTableData } from '../../blockchain';
...
async function getFactoryOnChain(factoryId: number): Promise<any> {
    ...
    results = await getTableData('eosio.nft.ft', 'eosio.nft.ft', 'factory.a', factoryId, factoryId); // providing the factoryId as upper and lower bound works as a "WHERE factory.id = {factoryId}" filter
    ...
    if (factoryIndex != -1) {
        ...
        results = await getTableData('eosio.nft.ft', 'eosio.nft.ft', 'factory.b', factoryId, factoryId);
        factoryIndex = results.rows.findIndex((x) => x.id == factoryId);
...
async function handleInteraction(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        const factoryInDb = await factoryDb.getFactory(factoryId);
        ...
        const resp = await factoryDb.addFactory(factoryId, role.id);
        if (!resp.status) {
            return interaction.editReply({
